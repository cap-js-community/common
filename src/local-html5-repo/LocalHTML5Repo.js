...
const defaultEnv = require(DEFAULT_ENV_PATH);
...
class LocalHTML5Repo {
  ...
  adjustDefaultEnv() {
    this.originalHtmlRepoUrl = defaultEnv.VCAP_SERVICES["html5-apps-repo"][0].credentials.uri;
    defaultEnv.VCAP_SERVICES["html5-apps-repo"][0].credentials.uri = `http://localhost:${this.port}`;
    this.writeDefaultEnv();
  }

  restoreDefaultEnv() {
    defaultEnv.VCAP_SERVICES["html5-apps-repo"][0].credentials.uri = this.originalHtmlRepoUrl;
    this.writeDefaultEnv();
  }

  writeDefaultEnv() {
    const url = defaultEnv.VCAP_SERVICES["html5-apps-repo"][0].credentials.uri;
